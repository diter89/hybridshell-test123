...
class StreamingUIManager:
    ...
    def stream_ai_response_with_live_markdown(
        ...
        api_call_func,
        *args,
        final_title: str = " AI Assistant - Complete",
        ...
        **kwargs,
        ...
        with Live(console=self.console, refresh_per_second=12) as live:
            try:
                live.update(
                    PanelTheme.build(
                        Align.center("󰀂 Establishing connection to AI service..."),
                        title=" AI Assistant",
                        style="info",
                        padding=(1, 2),
                        fit=True,
    ...
    def stream_shell_command(self, command: str, process: subprocess.Popen):
        ...
        if input_enabled:
            old_term_settings = termios.tcgetattr(stdin_fd)  # type: ignore[arg-type]
            tty.setcbreak(stdin_fd)  # type: ignore[arg-type]
        ...
        try:
            with Live(console=self.console, refresh_per_second=12) as live:
                live.update(
                    PanelTheme.build(
                        Align.center(f"󰀂 Executing: '{title_command}'"),
                        title=" Shell Command",
                        style="info",
                        padding=(1, 2),
                        fit=True,
                ...
                while True:
                    watch_fds = [stdout_fd]
                    if input_enabled:
                        watch_fds.append(stdin_fd)  # type: ignore[arg-type]
            ...
            if input_enabled and old_term_settings is not None:
                termios.tcsetattr(stdin_fd, termios.TCSADRAIN, old_term_settings)  # type: ignore[arg-type]
